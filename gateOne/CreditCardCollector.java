...
public class CreditCardCollector{
	public static void main(String [] args){
		...
		if(validOrInvalid(CardNumber))System.out.println("Invalid entry, check card number and retry");
			...
			System.out.printf("%n%s%s"**,"Credit Card Type :", type);
			System.out.printf("%n%s%s",**," Credit Card Digit length : " CardNumber.length());
			System.out.printf("%n%s%s", "Credit card Validity Status",validity);
			System.out.printf("%n%d*******************************");
		}
	...
	public static double getSecondDigitFromRightToLeft(String CardNumber){
		...
		public static String ValidityStatusChecker(String CardNumber){
			...
			double value = AddTotalOfOddPlacesAndSecondDigitFromRightToLeft(CardNumber) / 10;
			if(value == 0) checker = "Valid Card Number";
